...
const RestaurantGallery = ({ images }) => {
  ...
  return (
    <div className="restaurant-gallery">
      <button
        type="button"
        className="restaurant-gallery__current-img"
        onClick={() => openLightbox(currentImage)}
      >
        <img src={images[currentImagePreview].src} alt="restaurant-img" />
      </button>
      <div className="restaurant_gallery__gallery">
        {images.map((item, index) => (
          <button
            type="button"
            key={`index_${item.src}`}
            onClick={() => changeImg(index)}
            className="restaurant-gallery__img-preview"
          >
            <img src={item.src} alt="Restaurant-img" />
          </button>
        ))}
      </div>
      <Modal
